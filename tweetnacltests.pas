...
begin
  result := false;
  SetLength(m, crypto_secretbox_ZEROBYTES + Length(msg));
  FillChar(m[0], crypto_secretbox_ZEROBYTES, 0);
  Move(msg, m[crypto_secretbox_ZEROBYTES], Length(msg));
...
begin
  result := false;
  SetLength(c, crypto_secretbox_BOXZEROBYTES + Length(box));
  FillChar(c[0], crypto_secretbox_BOXZEROBYTES, 0);
  Move(box, c[crypto_secretbox_BOXZEROBYTES], Length(box));
...
begin
  result := false;
  SetLength(m, crypto_box_ZEROBYTES + Length(msg));
  FillChar(m[0], crypto_box_ZEROBYTES, 0);
  Move(msg, m[crypto_box_ZEROBYTES], Length(msg));
  SetLength(c,Length(m));
...
begin
  result := false;
  SetLength(c, crypto_box_BOXZEROBYTES + Length(box));
  FillChar(c[0], crypto_box_BOXZEROBYTES, 0);
  Move(box, c[crypto_box_BOXZEROBYTES], Length(box));
  SetLength(m,Length(c));
...
begin
  result := false;
  SetLength(sm, Length(signature) + Length(msg));
  Move(signature, sm[0], Length(signature));
  Move(msg, sm[Length(signature)], Length(msg));
...
function testHash():boolean;
const
  msg : array[0..42] of u8 = ($54,$68,$65,$20,$71,$75,$69,$63,$6B,$20,$62,$72,$6F,$77,$6E,$20,$66,$6F,$78,$20,$6A,$75,$6D,$70,$73,$20,$6F,$76,$65,$72,$20,$74,$68,$65,$20,$6C,$61,$7A,$79,$20,$64,$6F,$67);
  hash : array[0..crypto_hash_sha512-1] of u8 = ($07,$E5,$47,$D9,$58,$6F,$6A,$73,$F7,$3F,$BA,$C0,$43,$5E,$D7,$69,$51,$21,$8F,$B7,$D0,$C8,$D7,$88,$A3,$09,$D7,$85,$43,$6B,$BB,$64,$2E,$93,$A2,$52,$A9,$54,$F2,$39,$12,$54,$7D,$1E,$8A,$3B,$5E,$D6,$E1,$BF,$D7,$09,$78,$21,$23,$3F,$A0,$53,$8F,$3D,$B8,$54,$FE,$E6);
var
...
begin
  result := false;
    mlen := Length(msg);
    if crypto_hash(@h[0], @msg[0], mlen)<>0 then
      exit(false);
    for i:=0 to crypto_hash_sha512-1 do
      if h[i]<>hash[i] then
        exit(false);
